...
<!-- 				<view class="form_item">
				...
				<view class="form_mune" :class="'active'+muneindex">
				  <view class="mune_item"
					...
					@click="muneclick(index)" v-for="(item,index) in munelist" :key="index">{{i18n[item.name]}}</view>
				</view>
				<view class="form_item">
				...
				<view class="form_item">
					...
					<view class="form_input">
						<input type="number" v-model="amount" maxlength="13" name="amount" :placeholder="i18n.formtitle3_txt" />
					</view>
				</view>
...
export default {
	...
	methods: {
		...
		getwithdrawallist(){
			...
			this.$http.requestajx('user_withdrawal/withdravalMsg','get',{}).then((res) => {
...
.form_item{margin-bottom: 22px;
	...
	uni-input{	height: 35px;border-radius: 3px;border: solid 1px #e8e8e8;color: #333;padding: 0 8px;
	...
	.btn_red{color: #080d5d;text-decoration:underline;}
}
.forminfo{ font-size: 30upx;color: #f62f3d;}
.account {
	display: flex;
	align-items: center;
	width: 100%;
...
.subbtn{ padding-top: 50px;padding-bottom: 30px;
	.btn { margin-top: 0;
...
.form_mune{ border: 4px solid #F5F5F5;border-radius: 6px;overflow: hidden;display: flex;align-items: center;height: 38px;margin-top: 30px;position: relative;
