	...
	<view class="profile_back header_top">
		...
		<view class="header">
			<view class="header-info">
				<image :src="default_avatar" mode="" class="avatar" @click="uploadimg"></image>
		...
		<view class="withdraw_content">
			<view class="detailed">
				<view class="detailed-item" v-for="(item,name,index) in accountIndex" :key="index">
		...
		<view class="list">
			<uni-list>
				...
				<block v-for="(item,index) in navlistjson" :key="index" v-if="navlistjson[index]">
					<uni-list-item
...
export default {
	...
	data() {
		return {
			...
			navlistjson:[{imgsrc:'/static/images/profile/icon_@2.png',router_url:'/pages/profile/my-credit',name:0},
					{imgsrc:'/static/images/profile/icon_01_@2.png',router_url:'/pages/profile/personal-information',name:1},
					{imgsrc:'/static/images/profile/news.png',router_url:'/pages/profile/my-bill',name:3},
					{imgsrc:'/static/images/profile/icon_03_@2.png',router_url:'/pages/profile/my-team',name:4},
					{imgsrc:'/static/images/profile/icon_04_@2.png',router_url:'/pages/profile/change-password',name:5}],
			accountIndex: []
		};
	},
	computed: {
		...
		default_avatar(){
			return this.info.avatar.substring(0,5) == 'https' ? this.info.avatar : this.$configurl.ossBaseUrl + this.info.avatar;
		},
		socketonlinenum(){
			...
			uni.chooseImage({
			    ...
			    success: (res) => {
					...
					that.$http.uploadajax('upload',tempFilePaths[0],{},'image').then((res) => {
						// console.log(res);
						that.$http.requestajx('account/avatar','put',{avatar:res.result.full_uri}).then((data) => {
							// console.log(data)
							that.getAccountInfo();
							uni.hideLoading(); 
						});
...
.head_pos{ position: fixed;top: 0;left: 0;width: 100%;z-index: 999;background-color: #1b2664;}
...
.withdraw_content{ background-color: #ffffff;margin: -54rpx auto 0;border-radius: 20rpx;width: 680rpx;}
.withdraw_list{ display: flex;
	.withdraw_item{ margin: 20rpx 30rpx;width: 100%;text-align: center;padding: 10px 0;color: #fff;
