	...
	<view>
		...
		<view class="content">
			<view class="title_info" v-if="task_detail.commission > 0">
				<view class="info_top">
					{{i18n.commission}}：<text class="color_pink">{{task_detail.commission | formatPrice}}</text>
				</view>
				<view class="info_top">
					{{i18n.taskmoney}}：<text class="color_orange">{{task_detail.task_price | formatPrice}}</text>
				</view>
			</view>
		...
		<view class="shop_list">
			...
			<view class="table_content">
				<view class="munetable" v-for="(item,index) in i18ninfo.munetable" :key="index">
					<view class="table_item munetableleft">
						<text>{{item.title}}</text>
					</view>
					<view class="table_item munetableright">
						<view class="rtitle"><text class="title_icon">{{item.txt1}}</text></view>
						<view class="rtitle" :class="{active:index===0}">{{i18n.taskcomplete}}{{ i18n.halltask[task_detail.task_type] }}<label class="icon" v-if="index===0"><text class="icon_text"></text></label> 
						</view>
					</view>
				</view>
			</view>
...
.c-des{ padding-top: 10px;padding-right: 15px;color: #807f7f;font-size: 30rpx;display: flex;justify-content: space-between;
	.task_type{ color: #f325f3;font-size: 16px;}
	.task_des{ word-break: break-all;padding-right: 20px;}
}
.shop_list{
	...
	.munetable{ display: flex;justify-content: space-between;align-items: center;font-size: 26rpx;color: $colorgray;
		...
		.munetableright{ padding-left: 20px;
			.rtitle{ position: relative;
				...
				&.active{
					.icon{ border: 2px solid $colororgin;width: 15px;height: 15px;border-radius: 50%;display: inline-block;text-align: center;line-height: 15px;
				...
				.title_icon::before{ content:'';width: 1px;height: 28px;position: absolute;top: -31px;left: -16px;border-left: 2px dotted $colorgray;}
			}
		}
...
.content {
	...
	.title_info{
		...
		.color_pink{ color: #f325f3;}
		.color_orange{ color: #ffa006;}
	}
	.c-content {
		display: flex;
		width: 100%;
